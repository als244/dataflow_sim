...
`build_training_chain` is a convenience wrapper that applies PressureFit by
default.
...
def build_training_chain(
    ...
    *,
    ...
    """Backward-compatible wrapper: bare chain + PressureFit policy.

    Matches the old helper's high-level API. Equivalent to:
        bare = build_bare_training_chain(L, ...)
        return apply_pressurefit_policy(bare, device_capacity=...)

    `window_size` is retained for call-site compatibility with older examples;
    general policies do not use it.
    """
    # Imported here to avoid importing policy modules during lightweight
    # workload-math imports.
    ...
    bare = build_bare_training_chain(
        ...
        bandwidth_h2d=bandwidth_h2d,
        bandwidth_d2h=bandwidth_d2h,
        final_model_state_on_host=final_model_state_on_host,
    )
