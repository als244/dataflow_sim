...
@dataclass(frozen=True)
class HardwareEnv:
    peak_tflops: float        # P, compute throughput
    gpu_membw_gbs: float      # M, GPU HBM bandwidth (GB/s)
    interconnect_bw_gbs: float  # host<->device link (GB/s)
    matmul_eff: float
    attn_fwd_eff: float
    attn_bwd_eff: float
    mem_eff: float = 0.9
...
def gbs_to_bytes_per_microsecond(gbs: float) -> int:
    """Bytes/µs from GB/s. 1 GB/s = 1e9 B / 1e6 µs = 1000 B/µs."""
...
def _eff_value(hw: HardwareEnv, eff_name: EffName) -> float:
    if eff_name == "none":
        return 1.0
    return getattr(hw, _EFF_LOOKUP[eff_name])


def time_subop(subop: SubOp, hw: HardwareEnv) -> SubOpTiming:
...
def layer_fwd_breakdown(spec: TransformerSpec, hw: HardwareEnv,
                        cfg: TrainingConfig) -> list[SubOpTiming]:
    return [time_subop(s, hw) for s in forward_subops(spec, cfg)]


def layer_bwd_breakdown(spec: TransformerSpec, hw: HardwareEnv,
                        cfg: TrainingConfig) -> list[SubOpTiming]:
    return [time_subop(s, hw) for s in backward_subops(spec, cfg)]


def head_breakdown(spec: TransformerSpec, hw: HardwareEnv,
                   cfg: TrainingConfig) -> list[SubOpTiming]:
...
def layer_bwd_microseconds(spec: TransformerSpec, hw: HardwareEnv,
                           cfg: TrainingConfig) -> int:
