...
Examples:
    python app/scripts/pressurefit_mode_sweep.py --quick
    python app/scripts/pressurefit_mode_sweep.py --compact --optimizer adamw --num-steps 2
    python app/scripts/pressurefit_mode_sweep.py --canonical --out /tmp/pf_modes.csv
...
from dataflow_sim.policy.pressurefit import PressureFitPortfolioMode, plan_pressurefit_policy
...
def _run_one(args, hw_name: str, cap_gb: int | None, M: int, S: int, mode: PressureFitPortfolioMode) -> dict:
    ...
    try:
        bare, _breakdown = build_transformer_bare_chain(spec, hw, cfg)
