...
def test_compute_waits_for_needed_queued_inputs_not_unneeded_tail():
    ...
    live, not after later H2D queue entries that the task does not need."""
    chain = _xfer_chain(
        [
            {"id": "X", "size": 10, "location": "host"},
            {"id": "A", "size": 10, "location": "host"},
            {"id": "B", "size": 10, "location": "host"},
            {"id": "C", "size": 10, "location": "host"},
        ],
        [
    ...
    assert intervals["h2d:X"] == (1, 11)
    assert intervals["h2d:A"] == (11, 21)
    assert intervals["h2d:B"] == (21, 31)
    assert intervals["h2d:C"] == (31, 41)
