...
from dataflow_sim.policy.min_grow import (
    Interval,
    Plan,
    _build_facts,
    _derive_schedule,
...
def test_smart_prefetch_returns_int_in_range():
    ...
    from dataflow_sim.policy.min_grow import _smart_prefetch_task
    bare = TaskChain(
        initial_memory=[Object(id="W", size=10, location="host", type="weight")],
        tasks=[
...
def test_analytic_pre_pass_reaches_static_feasibility():
    ...
    from dataflow_sim.policy.min_grow import _greedy_shrink_to_static_cap, _max_plan, _static_peak
    bare = TaskChain(
        initial_memory=[
